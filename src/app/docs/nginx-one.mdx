...
1. Now save the data plane key, so it can be used in further steps.
...
Now that we have a data plan key, we can attach NGINX instances to the NGINX One Console.

We have provided a few NGINX instances below for you to use for the remainder of this lab.  This lab will focus more on the NGINX One Console and less on how to install NGINX Agent on your NGINX instance and connect to NGINX One.  If this is something you would like to explore in greater depth, check out the lab appendix.

1. Click the "Run" button for each of your NGINX instances to deploy them and have them automatically connect to the NGINX One Console:

    <DockerContainer>
    <Docker 
        name="nginx-plus-1" 
        desc="NGINX Plus with Agent." 
    ...
    <Docker 
        ...
        desc="NGINX OSS with NGINX Agent."
...
1. You should see your new instances in the list. Click a hostname to view the instance details.
