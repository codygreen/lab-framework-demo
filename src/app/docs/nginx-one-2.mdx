...
We'll get to explore this in greater detail later in the lab when we configure NGINX to use a TLS certificate.
...
In this section, we will configure our **<GetVariable name="nginx-plus-1" />** NGINX instance to leverage an existing TLS certificate for encrypted communication. Later, we will perform more advanced certificate functions in [Part 4](./nginx-one-4.mdx) of this lab.
...
As part of your day-to-day tasks, you may need to remove NGINX instances from a Config Sync Group. As a precaution, you can only remove offline instances from a Config Sync Group. This section will walk you through this process.

To remove an NGINX Plus instance from this Config Sync Group:

1. Click the *Stop* button below to stop the NGINX Plus 3 instance, which will mark it offline in the console.
    ...
    > **Note:** If the instance above shows a green **Run** button, you will need to refresh the lab guide page, then click the *Stop* button.
...
In this lab, much of the work to connect instances to the NGINX One Console has been done for you. Part 3 of this lab is focused on manually connecting NGINX instances to the console, and is *optional*. If you would like to continue to this optional portion of the lab, click [here](./nginx-one-3.mdx).

Otherwise, Part 4 of the lab is focused on the Certificate Management features of the console. [Proceed to Part 4 of the lab](./nginx-one-4.mdx).
